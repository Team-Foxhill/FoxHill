...
namespace FoxHill.Monster.AI
{
    ...
    public class SouthBossMonsterController : MonoBehaviour, IDamager, IDamageable, IStat, IStaggerable, IPausable
    {
        ...
        private void PerformAttack(AnimationEvent animationEvent) // 애니메이션 이벤트로 받는 메서드.
        {
            ...
            {
                tmp1 = attackSpec.CastCenterPosition - new Vector2(2f, 0);
                if (_machine.CurrentState == State.AttackVertical)
                {
                    tmp1 -= new Vector2(.86f, 0);
                }
            }
            else
            {
                tmp1 = attackSpec.CastCenterPosition;
            }
            RaycastHit2D[] hit2D = Physics2D.CircleCastAll((Vector2)transform.position + tmp1, attackSpec.Radius, attackSpec.Direction, attackSpec.Distance, _playerMask | _towerMask);
        ...
        private void OnDrawGizmos()
        {
            ...
            if (tmp != null)
                Gizmos.DrawWireSphere((Vector2)transform.position + tmp1, tmp.Radius);
        }
